...
init(InitArgs) ->
    ...
    tlser:log(info, "tlser_session_cache> init/1 called with role: ~p~n", [Role]),
    Name =
        case Role of
            server ->
                ?SERVER_CACHE;
    ...
    tlser:log(info, "tlser_session_cache> Created new ETS table ~p~n", [Name]),
    ...
    case ets:info(?RESUMPTION_TRACKER, name) of
        undefined ->
            ...
            tlser:log(info, "tlser_session_cache> Created resumption tracker table~n", []);
        _ ->
            ok
    end,
    Name.
...
lookup(Name, Key) ->
    tlser:log(info, "tlser_session_cache> Session lookup called with key: ~0P~n", [Key, 10]),
    case ets:lookup(Name, Key) of
        [{Key, Session}] ->
            tlser:log(info, "tlser_session_cache> Session lookup: FOUND session for key ~0P~n", [
                Key, 10
            ]),
...
update(Name, Key, Session) ->
    tlser:log(info, "tlser_session_cache> Session update called with key: ~0P~n", [Key, 10]),
    ets:insert(Name, {Key, Session}),
    tlser:log(info, "tlser_session_cache> Session update: STORED session for key ~0P~n", [Key, 10]),
    % Debug: show session ID if available
    case Key of
        {_PartialKey, SessionId} ->
            tlser:log(info, "tlser_session_cache> Session ID in key: ~0P (length: ~p)~n", [
    ...
    tlser:log(info, "tlser_session_cache> Cache size after update: ~p~n", [CacheSize]),
    true.

delete(Name, Key) ->
    ets:delete(Name, Key),
    tlser:log(info, "tlser_session_cache> Session delete: removed session for key ~0P~n", [Key, 2]),
    true.
...
select_session(_Cache, _Key) ->
    tlser:log(info, "tlser_session_cache> select_session returns []~n", []),
    [].
