...
def get_session_ticket(session, debug=False):
    """Extract TLS 1.3 session ticket bytes from a Python SSLSession."""
    if not session:
        return None

    if debug:
        print(f"  [DEBUG] Session type: {type(session)}")
        print(f"  [DEBUG] Session dir: {[x for x in dir(session) if not x.startswith('__')]}")

    # Check if session has a ticket
    if hasattr(session, 'has_ticket'):
        if not session.has_ticket:
            if debug:
                print(f"  [DEBUG] Session has_ticket=False")
            return None

    # Method 1: Try to get ticket attribute directly (Python 3.7+)
    # TLS 1.3 sessions have a 'ticket' attribute containing the ticket bytes
    if hasattr(session, 'ticket'):
        ticket = session.ticket
        if debug:
            print(f"  [DEBUG] Found 'ticket' attribute: {type(ticket)}, value: {ticket[:20] if isinstance(ticket, bytes) and len(ticket) > 20 else ticket}")
        if isinstance(ticket, (bytes, bytearray)) and ticket:
            return bytes(ticket)

    # Method 2: Try to access ticket via _ticket (internal attribute)
    if hasattr(session, '_ticket'):
        ticket = session._ticket
        if debug:
            print(f"  [DEBUG] Found '_ticket' attribute: {type(ticket)}, value: {ticket[:20] if isinstance(ticket, bytes) and len(ticket) > 20 else ticket}")
        if isinstance(ticket, (bytes, bytearray)) and ticket:
            return bytes(ticket)

    # Method 3: Try to get ticket from session's internal state
    # Some Python versions store it differently
    if hasattr(session, '__dict__'):
        if debug:
            print(f"  [DEBUG] Session __dict__ keys: {list(session.__dict__.keys())}")
        for key, value in session.__dict__.items():
            if 'ticket' in key.lower():
                if debug:
                    print(f"  [DEBUG] Found ticket-related key '{key}': {type(value)}, value: {value[:20] if isinstance(value, bytes) and len(value) > 20 else value}")
...
def get_ticket_size(session, debug=False):
    """Extract TLS 1.3 session ticket size."""
    ticket = get_session_ticket(session, debug=debug)
    if ticket is None:
        return None
    return len(ticket)
...
def test_tls13_resumption(host1, port1, host2, port2, cert_paths, check_hostname=True):
    ...
    # Generate a unique client ID for server tracking (used in both connections)
    import uuid
    ...
    if session1:
        ...
        # Extract and store ticket bytes (in-memory)
        ticket1 = get_session_ticket(session1, debug=False)
        ticket_size = len(ticket1) if ticket1 is not None else None
        if ticket_size is not None:
            print(f"  Ticket size: {ticket_size} bytes")
            print("  Ticket stored in memory for resumption")
        else:
            print("  Ticket size: Unable to determine (Python ssl may not expose ticket bytes)")
    else:
    ...
    # Send a ping message so server can track this session
    ping_msg = f"ping-{client_id}".encode('utf-8')
    try:
        ssl_sock1.setblocking(True)
        ssl_sock1.settimeout(10)
        bytes_sent = ssl_sock1.send(ping_msg)
        print(f"Sent ping-{client_id} ({bytes_sent} bytes)")
        # Wait for pong response
        response = ssl_sock1.recv(1024)
        if response == b"pong":
            print(f"Received pong")
        else:
            print(f"Warning: Expected 'pong', got: {response}")
        # Keep connection open a bit longer to ensure server finishes processing
        time.sleep(0.5)
    except Exception as e:
        print(f"Warning: Failed to send ping/receive pong: {e}")
    ...
    try:
        ...
        session2 = tls_info2.get('session')

        # Send a ping message on the second connection so server can verify resumption
        # Use the same client ID so server knows it's the same client
        ping_msg = f"ping-{client_id}".encode('utf-8')
        try:
            ssl_sock2.setblocking(True)
            ssl_sock2.settimeout(10)
            bytes_sent = ssl_sock2.send(ping_msg)
            print(f"Sent ping-{client_id} ({bytes_sent} bytes)")
            # Wait for pong response - this ensures server has time to process and check resumption
            response = ssl_sock2.recv(1024)
            if response == b"pong":
                print(f"Received pong")
            else:
                print(f"Warning: Expected 'pong', got: {response}")
        ...
        # Print ticket sizes for comparison
        ticket_size1 = get_ticket_size(session1)
        ticket_size2 = get_ticket_size(session2) if session2 else None
        print("")
        print("--- Ticket Size Comparison ---")
        if ticket_size1 is not None:
            print(f"First connection ticket size:  {ticket_size1} bytes")
        else:
            print("First connection ticket size:  Unable to determine")
        if ticket_size2 is not None:
            print(f"Second connection ticket size: {ticket_size2} bytes")
        else:
            print("Second connection ticket size: Unable to determine")
        print("")
